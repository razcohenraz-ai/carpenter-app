...
**ההחלטה — שדה `CabinetInput.cornerFiller`** (`{ doorSide, doorWidthCm, returnDepthCm }`, אופציונלי; נוכחות = גוף פינה). מקור-יחיד לגאומטריה: **`core/product/cornerModule.ts`** (`isCorner`, `cornerFrontXLayout`, `cornerHingeSide`, `cornerFillerCutItems`, `cornerReturnBox`). מושחל דרך כל הצרכנים (Gotcha #8): לולאות הדלת ב-`cabinetCompute`+`useCabinet` (רוחב→`doorWidthCm`, hingeSide→צד המילוי), חיתוכי המילוי בשניהם, `cabinetFrontPanels` (דלת+מילוי → overlay 2D + מבט-3D), `cabinetBoards3D` (לוח הזקף), ו-5 צרכני decompose.

**`noWidthSplit` ב-`decomposeBoxes`** (החלטה נלווית, התגלתה במימוש): `MAX_BOX_W=100` פיצל את גוף ה-125 לשתי תיבות (62.5 כ"א) → שתי דלתות, והמדפים (`single:single`) לא נתלו. פתרון: פרמטר `noWidthSplit` ששומר גוף יחיד; מועבר `isCorner(input)` / prop `cornerSingleWidth` בכל מקום שמפרק תיבות (cut ×2, 3D, 2 sketch builders).

**החלטות נגריות שאושרו**: (א) המילוי = **שני חיתוכים מלאים** (פנים מלא-גובה-דלת + זקף `returnDepthCm`×גובה-פתח-פנימי), שניהם חומר חזית, מחוברים בחיתוך 90° miter; (ב) הצירים **תמיד בצד המילוי**; (ג) צד הדלת ברירת מחדל **ימין**; (ד) הכל עריך (צד/רוחב/עומק/מדפים). הפנים מסויף כמו דלת, הזקף גלם.

---

## 2026-06-16 — תצוגת חדר שלב 3: מבט תלת-ממד (react-three-fiber) על אותו מקור

**ההקשר**: שלב 3 (סיום הרצף 1→2→3). הנגר אישר להתקדם ל-3D גס — לא כפיצ'ר ליטוש אלא ככלי **אימות**: ה-3D הוא המבט היחיד שמפעיל עומק+גובה יחד, ולכן חושף שגיאות שמבטי ה-2D (כל אחד מקריס ציר) מסתירים מבנית.

**`placementSubBoxAABBs` כ-API ציבורי (מקור יחיד)**: ה-transform המקומי→חדר (`subBoxRoomAABB`) שהיה private ושירת רק את החזית — נחשף כפונקציה ציבורית שמחזירה `RoomAABB[]`. `placementElevationRects` עכשיו צורך אותו (החזית = היטל טהור), וה-3D מרנדר תיבה לכל AABB. כך **שלושת המבטים = מודל אחד**: `productSubBoxes` (local) → `placementSubBoxAABBs` (room) → top=AABB / elevation=projection / 3D=mesh. parity test נועל ש-elevation.h/y == AABB.x/y.

**למה r3f ולא three גולמי**: react-declarative (תיבות = JSX), OrbitControls מוכן ב-drei, אינטגרציה טבעית עם React 19 (r3f v9). הקומפוננטה צורכת את אותו core — אין לוגיקת מיקום ב-component.

**lazy-load**: `RoomView3D` נטען ב-`React.lazy` — three.js (~900KB) הוא chunk נפרד שיורד רק כשפותחים את טאב התלת-ממד; ה-bundle הראשי נשאר ~418KB (לא מעניש משתמשים שלא נכנסים ל-3D).

**יחידות בלי scaling**: ה-3D מרנדר ב-ס"מ ישירות (אותם מספרים כמו ה-AABB), המצלמה/אורות מכוילים ל-span. לא מתרגמים למטרים — שומר על debug פשוט והתאמה 1:1 לערכי ה-core.

**תוסף בלבד**: אין שינוי types/schema/serialize. הפאנל לעריכה (קיר/Y/סיבוב) פעיל גם ב-3D. 725 טסטים עוברים (4 חדשים ל-`placementSubBoxAABBs`).
