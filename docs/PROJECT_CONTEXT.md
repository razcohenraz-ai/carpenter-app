...
- **עורך הגוף (לחיצה על גוף) + עקיפת חומרים פר-body** — לחיצה על גוף → `BoxInteriorEditor` **ישירות** (BodyView הוסר). העורך כולל **4 לשוניות** (גופים/חזיתות/חיתוכים/פרזולים, משותפות 2D+3D — חזיתות = גופים + שכבת דלתות; חיתוכים/פרזולים קריאה-בלבד דרך `onlyBoxStableKey`), עקיפת מידות W/H/D, **עקיפת חומר גוף/חזית + עובי-גב פר-body מתחת לעקיפת המידות**, עריכת פנים, ועקיפת קנט. `SavedCabinetState.boxMaterialOverrides` נפתר ב-`core/boards/boxMaterials.ts` (`resolveBoxMaterials`) ומושחל בכל מסלולי בניית-הלוחות (cut/2D/3D). חזיתות הגוף ל-3D נבנות ב-`cabinetFrontBoxes` (יוצא מ-`cabinetBoards3D.ts`). ה-shell וה-carcassD המשותפים נשארים על חומר-החזית של הארון. **במטבח (`kitchenDirectEdit`):** מודול נפתח **ישירות בעורך** (הטופס הוסר); בקרות-היחידה (מעטפת-צד, מרווח-דלתות, רוחב-חזית מרבי, מעטפת-קיר + בורר קלפה, בקרות-פינה) קופלו לסקציית "הגדרות יחידה" מתחת ללשוניות, מחשבות-חי; "חזרה" יוצא למטבח. **לשונית "חזיתות" אינטראקטיבית** (`CabinetFrontsSketch`): סימוני-ציר + לחיצה על חזית פותחת `DoorEditor`/`ExternalDrawerEditor` — כמו בארון. הלשונית הפעילה נשלטת מ-`CabinetForm` (`tab`/`onTabChange`) ושורדת remount של עריכת-דלת.
