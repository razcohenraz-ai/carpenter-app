...
(role census) של מסלול ה-3D מול רשימת החיתוך (מקור האמת) — בדיוק מחלקת ה-drift של
Gotcha #8. **הוסף את המודול החדש למטריצת ה-`CASES`** שם (input + state), כך
שכל לוח שה-3D מפיל/מכפיל/מתייג-לא-נכון נתפס אוטומטית. (הערה: הרשת לא מכסה את צביעת
הלוחות ב-2D שעדיין יושבת ב-`CabinetSketch.tsx` — ראה ההערה בתחתית הקובץ.)
