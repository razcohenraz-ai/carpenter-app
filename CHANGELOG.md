...
- **בדיקת drift אוטומטית ל-Gotcha #8** (שלושת אתרי בניית-הלוחות שעלולים להיפרד). `src/core/renderParity.test.ts` משווה, על פני מטריצת 13 ארונות מייצגים (בסיס, shell+מעטפת, דו-שורתי, ותשעת מודולי המטבח כולל קלפה עם/בלי מעטפת, ופינה), את **מפקד הלוחות-לפי-תפקיד (role census)** של מסלול ה-3D (`cabinetBoardBoxes`) מול **רשימת החיתוך** (`computeUnitCutsAndHardware` — מקור האמת). לוח שמסלול רינדור מפיל / מכפיל / מתייג-לא-נכון שובר את הבדיקה מיד (בדיוק הבאג של מעטפת הקלפה החסרה).
