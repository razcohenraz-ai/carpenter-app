...
### נוסף — פריסת לוחות (טאב "פריסה")

- טאב חדש **פריסה** לצד bodies/fronts/cuts/hardware (בארון הבודד וב-`KitchenOverview`) — מציג את רשימת החיתוכים **פרוסה על לוחות שלמים**, מקובצת לפי חומר (ולפי עובי — גב 6מ"מ נפרד מדופן 18מ"מ).
- מנוע `core/cuts/sheetLayout.ts` (טהור, נבדק) — **guillotine nesting** (shelf/strip דו-שלבי), לא free 2D packing: כל פריסה ניתנת לחיתוך במסור פאנל בקו ישר מקצה-לקצה. שיקולים: **קרף 3מ"מ** בין חלקים, **1ס"מ יישור** מכל מימד (שטח שמיש 243×121 מלוח 244×122).
- **כיוון סיב**: סיב הלוח לאורך הצד הארוך (244). **נעילת סיבוב** = כיבוד סיב — כל חלק מונח עם צדו הארוך מקביל ל-244 (ללא סיבוב מחוץ-לסיב); כשפתוח, מותר סיבוב 90° לניצולת. הלוח מצויר אנכי (סיב מלמטה למעלה).
- תצוגת `LayoutView`: SVG לכל לוח (חלקים בקנה-מידה + תוויות), ניצולת לכל לוח/חומר, כפתור **נעילת סיבוב** גלובלי, ובחירת לוח **244×122 / 305×122** לכל חומר. חלק גדול מהשטח השמיש → מסומן כ-oversize (לא מונח בכוח) עם רמז לעבור ללוח 305.
- נגזר on-the-fly מ-`result.cuts` (SSOT — לא נשמר); צרכן חדש של רשימת החיתוכים, ללא שינוי בחישוב קיים. Invariants נבדקים: כיסוי מלא, ללא חפיפה, בתוך השטח השמיש, guillotine-decomposable.
- **בדיקת parity** (`sheetLayout.parity.test.ts`): לכל cut list אמיתי, מספר החלקים בפריסה (מונחים + oversize) = מספר החלקים ברשימת החיתוכים. נבדק על 5 ארונות (מדפים + מגירות) ו-5 מטבחים המכסים את כל 9 מודולי המטבח, בשני מצבי הסיב.

### נוסף — תיעוד הנדסי חי: dependency + data-flow + QA + impact analysis

- 8 מסמכי `docs/` חדשים המתארים את הארכיטקטורה ההנדסית (חישובים/מודלים/renderers), לתחזוקה שוטפת עם התפתחות הקוד — לא תיעוד מימוש נקודתי:
  ...
  - **`QA_REGRESSION.md`** — בסיס-ידע רגרסיות חי: כל באג משמעותי → root cause + engineering assumption שנשבר + invariant + הבדיקה השומרת. מאותחל עם 9 רגרסיות מתועדות (REG-001…009), template, סטטיסטיקות, וכללי תחזוקה.
- `ARCHITECTURE.md` — נוסף מצביע לכל מסמכי הניתוח בראש הקובץ (discoverability מזרימת קריאת-הסשן).
...
- ה-flyout כולל כעת גם **עובי גב (מ"מ)** — ערך משותף נגזר מהיחידות (או "מעורב" כשנבדלות); שינוי כותב לכל היחידות דרך `setAllMaterials({ backThickness })` (נשמר ב-ס"מ ב-`input.backThickness`).

### תוקן — מעטפת לא הוצגה בתצוגת עורך גוף המטבח

- ה-input של תצוגת העורך (`bodyInput3d`) קיבע `hasShell/Left/Right=false`, ולכן הדלקת ה"מעטפת" ביחידת מטבח עודכנה ברשימת-החיתוכים ובסקירת-המטבח אך **לא הוצגה בתצוגת התלת-ממד של העורך**. כעת ב-`kitchenDirectEdit` ה-flags של המעטפת נלקחים מה-form, וה-W מורחב בעובי הצדדים העטופים כך שהקרקס הפנימי נשאר תואם ל-`editingBox.W` (הרוחב החיצוני של היחידה קבוע לאורך ההדלקה/כיבוי). הארון העצמאי נשאר ללא מעטפת בעורך-הגוף (מעטפת היא ברמת-הארון). *הערה: סקיצת ה-2D של עורך-הגוף עדיין משמיטה את המעטפת בכוונה — מוצגת בתלת-ממד.*

### שונה — עורך גוף המטבח: כפתור "מעטפת" + הסתרת רווח-דלתות ורוחב-חזית-מרבי

- ב-**עורך גוף המטבח** (`kitchenDirectEdit`) נוסף **כפתור "מעטפת"** לסרגל הכלים הקומפקטי — flyout עם מעטפת שמאל/ימין (+ מעטפת קלפה ביחידת wall), במקום ה-checkboxes השורתיים שהיו ב"הגדרות יחידה". מועבר כ-prop חדש `shellControls` ל-`BoxInteriorEditor` (מוצג רק כשניתן — לארון עצמאי המעטפת היא ברמת-הארון ולכן אין כפתור).
- **שדות "רווח בין דלתות" ו"רוחב מקסימלי לחזית" הוסתרו** מעורך גוף המטבח. הערכים נשמרים ב-state וממשיכים לזרום ל-`calculate()` — רק הפקדים הוסרו.
- מקטע "הגדרות יחידה" מוצג כעת רק כשנותר בו תוכן (מנגנון הרמה / פינה); ליחידה רגילה הוא נעלם.

### שונה — סרגל כלים קומפקטי בעורך הגוף (עקיפות + הוספה)

- מקטעי העקיפה תמיד-פתוחים (קנט / מידות / חומרים) ושורת ההוספה בעורך הגוף (`BoxInteriorEditor`) קובצו ל-**3 כפתורים** עם flyout, כמו בארון: לחיצה פותחת לצד/מתחת לכפתור, לחיצה נוספת או לחיצה מחוץ לסרגל מקפלת; קבוצה אחת פתוחה בכל רגע (`openTool` + click-outside).
  - **עקיפת מידות** — W/H/D + איפוס.
  - **עקיפת חומרים** — חומר גוף, חומר חזית, עובי גב, **+ עקיפת קנט** (ירושה/מותאם + עובי + גמר) + איפוס.
  - **הוספה** — מדף / מגירה / מוט / מחיצה (רק בגוף בודד; בגוף עם מחיצה כל תא שומר על שורת ההוספה שלו).
- כפתור עם עקיפה פעילה מסומן בנקודת-הדגשה. שורת ההוספה הישנה בעמודת הבקרות הוסרה (עברה ל-flyout).

### שונה — סרגל קלט קומפקטי בארון הראשי (כפתורי קבוצה + flyout)

- שדות הקלט של הארון הראשי (`CabinetForm`) קובצו ל-**4 כפתורים** בסרגל צד צר; לחיצה פותחת flyout עם השדות **לצד** הכפתור, לחיצה נוספת מקפלת. רק קבוצה אחת פתוחה בכל רגע (`openPanel`).
  - **מידות** — גובה, רוחב, עומק.
  - **חומרים** — חומר גוף, חומר חזית, עובי גב, עובי קנט, גמר קנט.
  - **מעטפת** — מעטפת חיצונית, לוח תקרה, דלת מכסה צוקל (+ מעטפת קלפה ומנגנון הרמה ביחידות wall).
  - **מבנה** — גובה צוקל, דלתות לגובה (+ גבהי קומות מותנים), רווח בין דלתות (+ בקרות פינה ביחידת פינה).
- **רוחב מקסימלי לחזית** (`maxDoorWidth`) **הוסתר** מה-UI זמנית — הערך נשמר ב-state וממשיך לזרום ל-`calculate()`; רק הפקד הוסר עד החלטה היכן למקמו.
- העמודה השמאלית צומצמה (190px במקום 320px) → יותר מקום לסקיצה. ה-flyout נפתח בצד ה-inline-end (לוגי, תקין ב-RTL/LTR); במסך צר נופל למצב inline מתחת לכפתור.
- **הסרגל נדבק לראש המסך בגלילה** (`.formCol` `position: sticky`, `top: var(--header-height)`, z-index מעל לשוניות הסקיצה ומתחת ל-header הגלובלי); במסך צר ה-sticky מבוטל.

- **עורך הגוף** (`BoxInteriorEditor`): כפתור "חזרה", הכותרת והלשוניות (גופים/חזיתות/חיתוכים/פרזולים) עטופים ב-`stickyBar` — נשארים בראש אזור-הגלילה בכל המסכים (גוף בודד, מחיצה, עריכה-ישירה במטבח).
- **הארון הראשי** (`CabinetForm`): לשוניות המצב (גופים/חזיתות/חיתוכים/פרזולים) עטופות ב-`stickyTabs`.
- **המטבח הראשי** (`KitchenOverview`): ה-`toolbar` (לשוניות + מידות) `position: sticky`.
- כולן נדבקות מתחת ל-header הגלובלי (`top: var(--header-height)`), עם רקע אטום ו-z-index נמוך מה-header כדי לא לכסותו.

### שונה — גלילה בשתי אצבעות לא מזיזה/מקרבת בתצוגות 3D

- ב-3D של **תצוגת החדר** (`RoomView3D`) ושל **עורך הגוף** (`Body3DView`) — גלילה בשתי אצבעות (אירועי wheel) כבר לא מקרבת/מרחיקה את המצלמה (`OrbitControls enableZoom={false}`, הוסר `zoomToCursor`). סיבוב (גרירה) והזזה (pan) ממשיכים לעבוד. הערה: גם צביטה (pinch) לזום מושבתת כעת.

### נוסף — מידות החללים בין הפריטים בסקיצת-הגוף 2D

- **עורך הגוף (2D)**, **תצוגת הגופים הראשית של הארון** (לשונית "גופים") **וסקירת המטבח** (`KitchenOverview`, לשונית "גופים") מציגים כעת את **האור הפנוי (ס"מ) של כל חלל** בתוך הגוף — מהרצפה לפריט הראשון, בין כל שני פריטים, ומהפריט העליון לתקרה. שרשרת-מידה דקה מצוירת בתוך כל חלל ריק (לא חוצה מדף/מגירה/מוט). תקף לגוף בודד ולכל תא בגוף עם מחיצה.
- **מקור יחיד**: `computeInteriorGaps` (`core/interior/interiorUtils.ts`) משתמש באותם תחומים פיזיים של אזהרות-המרווח (גובה מגירה, עובי מדף = עובי חומר הגוף, ±1.5 ס"מ סביב מרכז המוט); אזורים חופפים מתמזגים (אין חלל-רפאים). מודפס דרך prop חדש `showGaps` ב-`BoxBodySketch` וב-`CabinetSketch` (נדלק רק בלשונית "גופים" — בארון ובמטבח — לא בחזיתות/חדר). בתצוגת הארון/מטבח נכללים גם המדפים המבניים של גוף מאוחד. 6 בדיקות חדשות.
