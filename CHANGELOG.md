...
- **עורך הגוף (2D)** וגם **תצוגת הגופים הראשית של הארון** (לשונית "גופים") מציגים כעת את **האור הפנוי (ס"מ) של כל חלל** בתוך הגוף — מהרצפה לפריט הראשון, בין כל שני פריטים, ומהפריט העליון לתקרה. שרשרת-מידה דקה מצוירת בתוך כל חלל ריק (לא חוצה מדף/מגירה/מוט). תקף לגוף בודד ולכל תא בגוף עם מחיצה.
- **מקור יחיד**: `computeInteriorGaps` (`core/interior/interiorUtils.ts`) משתמש באותם תחומים פיזיים של אזהרות-המרווח (גובה מגירה, עובי מדף = עובי חומר הגוף, ±1.5 ס"מ סביב מרכז המוט); אזורים חופפים מתמזגים (אין חלל-רפאים). מודפס דרך prop חדש `showGaps` ב-`BoxBodySketch` וב-`CabinetSketch` (בארון נדלק רק בלשונית "גופים", לא בחזיתות/מטבח/חדר). בתצוגת הארון נכללים גם המדפים המבניים של גוף מאוחד. 6 בדיקות חדשות.
